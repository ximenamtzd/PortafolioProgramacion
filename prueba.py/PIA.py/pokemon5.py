...
def validar_nombre(nombre):
    """Valida el nombre del Pokémon ingresado."""
    return bool(regex_nombre.fullmatch(nombre))

def obtener_datos_pokemon(nombre_pokemon):
    """Consulta la API y devuelve nombre, tipos y habilidades."""
    ...
    try:
        ...
        messagebox.showerror("Error de red", f"No se pudo conectar a la API: {e}")
    return None

def mostrar_info():
    """Muestra información del Pokémon y valida nombre."""
...
def guardar_archivos():
    """Guarda los datos en JSON, CSV y Excel."""
...
def leer_desde_excel():
    """Lee datos previamente guardados desde el archivo Excel."""
...
def validar_datos(pokemones):
    """Filtra y valida los datos leídos para asegurar su formato."""
    validos = []
    for p in pokemones:
        if (
...
def analizar_y_graficar():
    """Genera y guarda 4 tipos de gráficas con los datos actuales."""
    ...
    plt.tight_layout()
    plt.savefig("grafica_lineas_tipos.png")
    plt.show()
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig("grafica_barras_habilidades.png")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("grafica_pastel_tipos.png")
    plt.show()
